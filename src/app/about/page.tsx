import { Sparkles, Award, Users, CheckCircle, GraduationCap, ArrowRight } from "lucide-react";
import { getFaculty, getAffiliations } from "@/lib/sanity";

export default async function AboutPage({ searchParams }: { searchParams: Promise<{ category?: string; item?: string }> }) {
  const params = await searchParams;
  const activeCategory = params.category;
  const activeItem = params.item;
  ...
  return (
    <div className="bg-slate-50/50 min-h-screen py-16 md:py-24 select-none">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        {/* Dynamic Detail Viewer when a mega menu item is selected */}
        {activeItem ? (
          <div className="max-w-4xl bg-white border border-slate-200/60 p-8 md:p-12 shadow-xl shadow-indigo-50/20 rounded-3xl mb-16 hover:border-indigo-100 transition-all duration-300">
            <span className="inline-flex items-center gap-2 rounded-full bg-indigo-50 border border-indigo-100/60 px-4 py-1 font-sans text-xs font-bold text-indigo-700 tracking-wider">
              {activeCategory}
            </span>
            <h1 className="mt-6 font-outfit text-3xl font-black text-slate-900 md:text-5xl tracking-tight leading-none">
              {activeItem}
            </h1>
            <p className="mt-6 font-sans text-base md:text-lg leading-relaxed text-slate-600 max-w-3xl">
              St. Ann&apos;s College for Women holds absolute dedication to the highest standards of governance, statutory compliance, and institutional development. Information regarding <strong className="text-indigo-600">{activeItem}</strong> is curated to showcase our comprehensive, systematic approach to educational leadership and organizational transparency.
            </p>
            <div className="mt-12 bg-slate-50/60 border border-slate-100/80 rounded-2xl p-6 flex items-start gap-4 hover:bg-indigo-50/30 transition-colors duration-200">
              <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-tr from-indigo-500 to-indigo-700 text-white font-bold text-base shadow-sm">
                <CheckCircle className="h-5 w-5" />
              </span>
              <div>
                <h4 className="font-outfit font-black text-slate-800 text-base leading-tight">Authenticity & Quality Record</h4>
                <p className="font-sans text-xs font-semibold text-slate-500 leading-normal mt-1.5">
                  Maintained in complete alignment with IQAC and UGC guidelines for administrative excellence.
                </p>
              </div>
            </div>
          </div>
        ) : (
          /* Normal Default About page view if no item is active */
          <>
