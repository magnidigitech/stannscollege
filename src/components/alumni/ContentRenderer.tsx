...
import { staticAlumniSections } from "./staticData";
import { UserPlus, Heart, Users, Book, Award, Newspaper, LucideIcon } from "lucide-react";
...
export const ContentRenderer = ({ slug }: { slug: string }) => {
  const data = staticAlumniSections[slug];
      ...
      {data.quickLinks && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {data.quickLinks.map((link, idx) => {
            const Icon = link.icon ? iconMap[link.icon] : Award;
            return (
              <Link
                key={idx}
                href={link.slug.startsWith('/') ? link.slug : `/alumni/${link.slug}`}
                className="flex items-center gap-3 p-4 bg-white border border-slate-100 rounded-2xl shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group"
              >
                ...
                <span className="text-sm font-bold text-slate-700 group-hover:text-[#002147]">{link.text}</span>
              </Link>
            );
          })}
        </div>
          ...
          if (node.type === 'image') {
            return (
              <div key={idx} className="my-8 rounded-2xl overflow-hidden shadow-sm border border-slate-100">
                <img src={node.text} alt="Alumni content" className="w-full h-auto object-cover" />
              </div>
            );
          }
